...
export default function OptimisticSleepLog({ 
  sleepLog,
  ...
  const openModal = (_?: SleepLog) => {
    setOpen(true);
  };
  const closeModal = () => setOpen(false);
  const [optimisticSleepLog, setOptimisticSleepLog] = useOptimistic(sleepLog);
  const updateSleepLog: TAddOptimistic = (input) =>
    setOptimisticSleepLog({ ...input.data });
  ...
  return (
    <div className="m-4">
      ...
      <div className="flex justify-between items-end mb-4">
        <h1 className="font-semibold text-2xl">{optimisticSleepLog.date}</h1>
