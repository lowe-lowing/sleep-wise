...
export const getSleepLogs = async () => {
  ...
  const rows = await db.select().from(sleepLogs).where(eq(sleepLogs.userId, session?.user.id!));
  const s = rows
  return { sleepLogs: s };
};
...
export const getSleepLogById = async (id: SleepLogId) => {
  ...
  const [row] = await db.select().from(sleepLogs).where(and(eq(sleepLogs.id, sleepLogId), eq(sleepLogs.userId, session?.user.id!)));
